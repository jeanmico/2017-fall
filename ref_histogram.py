...
#gc_arr = np.genfromtxt(file_path, delimiter='\t', skip_header=1, dtype=['U15', 'U15', 'U15', 'i', 'f', 'f', 'f', 'f'], names=('id', 'bin_id', 'contig_id', 'length', 'contig_gc', 'mean_gc', 'std_gc', 'z_score')) #print(gc_arr)
#print(gc_arr[3999, 0])
#print(gc_arr[3999, 1])
#print(gc_arr.shape())
...
max_z = gc['z_score'].max()

# make a histogram of reference genome z-scores
plt.figure(0)
bins = np.linspace(0, max_z, 100)
...
axes = plt.gca()
axes.set_yscale('log')
axes.set_xlim(left=0, right=math.ceil(max_z))
